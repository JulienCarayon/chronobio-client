...
class Aigrisculteurs:
    ...
    def __init__(self: "Aigrisculteurs"):
        ...
        self.list_of_fiels = []
        self.tractor_data = []  # [tractor_id]{'worker':worker_id,'location':FIELD_ID or FACTORY_STOCK, destination:FIELD_ID or FACTORY_STOCK or None if not used}
        self.testing = False
        self.field_status = []
        self.day_bool = True
    ...
    def run(self, game_data, testing=False):
        ...
        try:
            ...
            if self.day == 0:
                ...
                self.send_group_to_place(workers_id_start=38, workers_id_length=8, place=FACTORY_SOUPE)
                ...
                self.new_day()
                self.send_group_to_place(workers_id_start=1, workers_id_length=11, place=1)
                ...
                self.send_group_to_place(workers_id_start=12, workers_id_length=11, place=3)
                ...
                self.send_group_to_place(workers_id_start=12, workers_id_length=11, place=4)
    ...
    def send_worker_to_place(self, place=FACTORY_STOCK, worker_id=None, tractor_id=None, field_to_collect=None):   # noqa: E501
        ...
        if place == FACTORY_SOUPE:
            ...
            logging.debug(f"WORKER {worker_id} ASKED TO {place} COLLECT ? {field_to_collect}")
            self.store_with_tractor(worker_id, tractor_id, field_to_collect)

            
        elif 1 <= int(place) <= 5:
            logging.debug(f"WORKER {worker_id} : IS FIELD{int(place)} SOWN? {self.check_if_field_sown(int(place))}")
            ...
            if tractor_available: 
                if field_collectable:
                    ...
                    if self.tractor_data[tractor_id - 1].get(WORKER) == worker_id or self.tractor_data[tractor_id - 1].get(WORKER) == None:
                        logging.debug(f"WORKER {worker_id} STORED {field_id} WITH {tractor_id}")
    ...
    def check_if_tractor_available(self,tractor_id):
        ...
        if tractor_id <= self.actual_number_of_tractors:
            if self.tractor_data[tractor_id - 1][LOCATION] == FACTORY_STOCK[1] \
    ...
    def update_tractor_position(self):
        for tractor in range (1,self.actual_number_of_tractors):
            ...
            logging.debug(f'TRACTOR {tractor} : LOCATION: {position} DESTINATION : {self.tractor_data[tractor][DESTINATION]}')
